...
node {
    ...
    withCredentials([file(credentialsId: JWT_KEY_FILE_CRED_NAME, variable: 'jwt_key_file')]) {
        ...
        stage('Deploy To Org') {
            ...
            if (isPrToMain) {
                rc = sh returnStatus: true, script: "${toolbelt}/sfdx force:source:deploy --checkonly --testlevel RunLocalTests --targetusername ${ORG_USERNAME} -p \"3-rd-party, ss_admin_console\""
                if (rc != 0) {
                    error 'Validation failed'
                }
            } else {
                rc = sh returnStatus: true, script: "${toolbelt}/sfdx force:source:deploy --targetusername ${ORG_USERNAME} -p \"3-rd-party, ss_admin_console\""
                if (rc != 0) {
                    error 'Deploy failed'
                }
            }
